...
func mustLogsFS(fs *flag.FlagSet) *logs.DataFS {
	if len(fs.Args()) != 1 {
		fs.Usage()
	}
	fsys, err := logs.NewDataFS(fs.Arg(0))
	if err != nil {
...
func main() {
	...
	subcommands.Register(new(hostEnforcerLogsCmd), "host-enforcer")
	subcommands.Register(new(hostEnforcerChanges), "host-enforcer")
