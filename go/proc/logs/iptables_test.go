...
`,
		...
		{
			name: "issue-2021-09-16",
			input: `
				# Generated by iptables-save v1.8.4 on Thu Sep 16 16:03:27 2021
				*mangle
				:PREROUTING ACCEPT [5:314]
				:INPUT ACCEPT [5:314]
				:FORWARD ACCEPT [0:0]
				:OUTPUT ACCEPT [6:716]
				:POSTROUTING ACCEPT [6:716]
				-A OUTPUT -d 192.168.1.14/32 -o eno50 -p tcp -m tcp -j CLASSIFY --set-class 0001:0004
				-A OUTPUT -d 192.168.1.14/32 -o eno50 -p tcp -m tcp -j DSCP --set-dscp 0x12
				-A OUTPUT -d 192.168.1.14/32 -o eno50 -p tcp -m tcp -j RETURN
				-A OUTPUT -d 192.168.1.15/32 -o eno50 -p tcp -m tcp -j CLASSIFY --set-class 0001:0004
				-A OUTPUT -d 192.168.1.15/32 -o eno50 -p tcp -m tcp -j DSCP --set-dscp 0x12
				-A OUTPUT -d 192.168.1.15/32 -o eno50 -p tcp -m tcp -j RETURN
				-A OUTPUT -d 192.168.1.2/32 -o eno50 -p tcp -m tcp -j CLASSIFY --set-class 0001:0002
				-A OUTPUT -d 192.168.1.2/32 -o eno50 -p tcp -m tcp -j DSCP --set-dscp 0x12
				-A OUTPUT -d 192.168.1.2/32 -o eno50 -p tcp -m tcp -j RETURN
				-A OUTPUT -d 192.168.1.3/32 -o eno50 -p tcp -m tcp -j CLASSIFY --set-class 0001:0002
				-A OUTPUT -d 192.168.1.3/32 -o eno50 -p tcp -m tcp -j DSCP --set-dscp 0x12
				-A OUTPUT -d 192.168.1.3/32 -o eno50 -p tcp -m tcp -j RETURN
				COMMIT
				# Completed on Thu Sep 16 16:03:27 2021
				`,
			output: []EnforcementRule{
				{DstIP: "192.168.1.14", QoS: "0x12", ClassID: TCHandle{1, 4}},
				{DstIP: "192.168.1.15", QoS: "0x12", ClassID: TCHandle{1, 4}},
				{DstIP: "192.168.1.2", QoS: "0x12", ClassID: TCHandle{1, 2}},
				{DstIP: "192.168.1.3", QoS: "0x12", ClassID: TCHandle{1, 2}},
			},
		},
	}
	...
	for _, test := range tests {
		...
		if err != nil {
			...
			if len(recs) != len(test.output) {
				t.Log("different # of rules")
				repErr()
			} else {
				for i := 0; i < len(test.output); i++ {
					if recs[i] != test.output[i] {
						t.Logf("mismatch in rule %d", i)
						repErr()
					}
				}
			}
