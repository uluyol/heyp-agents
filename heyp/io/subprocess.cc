/* Derived from
https://github.com/tensorflow/tensorflow/blob/5dcfc51118817f27fad5246812d83e5dccdc5f72/tensorflow/core/platform/default/subprocess.cc
...
void SubProcess::SetProgram(const std::string& file_raw,
                            const std::vector<std::string>& argv_raw) {
  std::vector<std::string> argv(argv_raw.size() + 1, "");
  argv[0] = file_raw;
  std::copy(argv_raw.begin(), argv_raw.end(), ++argv.begin());
