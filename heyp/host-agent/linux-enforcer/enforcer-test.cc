...
TEST(LinuxHostEnforcer, ChangedAllocsAndPri) {
  ...
  std::string change_rl_tc3 =
      "class change dev eth1 parent 1: classid 1:2 htb rate 500.000000mbit\n";
  std::string change_rl_tc4 =
      "class change dev eth1 parent 1: classid 1:3 htb rate 50.000000mbit\n";

  std::string change_ipt1 =
      "*mangle\n"
  ...
  std::string change_ipt2 =
      "*mangle\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.2 -j CLASSIFY --set-class 1:3\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.2 -j CLASSIFY --set-class 1:3\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.2 -j DSCP --set-dscp-class BE\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.2 -j DSCP --set-dscp-class BE\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.2 -j RETURN\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.2 -j RETURN\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.3 -j CLASSIFY --set-class 1:3\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.3 -j CLASSIFY --set-class 1:3\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.3 -j DSCP --set-dscp-class BE\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.3 -j DSCP --set-dscp-class BE\n"
      "-D OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.3 -j RETURN\n"
      "-D FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.3 -j RETURN\n"
      "-A OUTPUT -o eth1 -p tcp -m tcp -d 10.0.0.2 -j CLASSIFY --set-class 1:2\n"
      "-A FORWARD -o eth1 -p tcp -m tcp -d 10.0.0.2 -j CLASSIFY --set-class 1:2\n"
  ...
  auto& tc_exp3 = EXPECT_CALL(*tc_caller, Batch(absl::Cord(change_rl_tc2), true))
                      .Times(1)
                      .After(tc_exp2);
  auto& tc_exp4 = EXPECT_CALL(*tc_caller, Batch(absl::Cord(change_rl_tc3), true))
                      .Times(1)
                      .After(tc_exp3);
  EXPECT_CALL(*tc_caller, Batch(absl::Cord(change_rl_tc4), true)).Times(1).After(tc_exp4);
  EXPECT_CALL(*tc_caller, Call(_, _)).Times(2);
  EXPECT_CALL(*tc_caller, RawOut()).Times(0);
  EXPECT_CALL(*tc_caller, GetResult()).Times(0);
  ...
  auto& ipt_exp2 =
      EXPECT_CALL(*ipt_runner,
          ...
          .After(ipt_exp1);
  auto& ipt_exp3 =
      EXPECT_CALL(*ipt_runner, Restore(iptables::Table::kMangle, absl::Cord(change_ipt1),
                                       iptables::RestoreFlags{.flush_tables = false,
                                                              .restore_counters = false}))
          .Times(1)
          .After(ipt_exp2);
  auto& ipt_exp4 =
      EXPECT_CALL(*ipt_runner, Restore(iptables::Table::kMangle, absl::Cord(change_ipt2),
                                       iptables::RestoreFlags{.flush_tables = false,
                                                              .restore_counters = false}))
          .Times(1)
          .After(ipt_exp3);
  ...
  proto::AllocBundle allocs3 = ParseTextProto<proto::AllocBundle>(R"(
    flow_allocs {
      flow { src_dc: "A" dst_dc: "B" }
      hipri_rate_limit_bps: 524288000
    }
  )");
  proto::AllocBundle allocs4 = ParseTextProto<proto::AllocBundle>(R"(
    flow_allocs {
      flow { src_dc: "A" dst_dc: "B" }
      lopri_rate_limit_bps: 52428800
    }
  )");
  ...
  state.enforcer->EnforceAllocs(MockFlowStateProvider(), allocs1);
  state.enforcer->EnforceAllocs(MockFlowStateProvider(), allocs2);
  state.enforcer->EnforceAllocs(MockFlowStateProvider(), allocs3);
  state.enforcer->EnforceAllocs(MockFlowStateProvider(), allocs4);
}
