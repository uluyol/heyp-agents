...
for (be in unique(data$BE)) {
  pdf(paste(outpre, gsub("/", "_", be), ".pdf", sep=""), height=2.5, width=5)
  p <- ggplot(data[data$BE == be,], aes(x=WantGbps, y=LatencyNanos/1e6, color=factor(Perc))) +
      geom_line(size=1) +
      xlab("Intended Goodput (Gbps)") +
      ylab("Latency (ms)") +
